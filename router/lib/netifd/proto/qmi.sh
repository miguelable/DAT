...
proto_qmi_setup() {
	...
	[ "`cat /tmp/sim_type 2>/dev/null`" = "verizon" ] && pdptype="ipv4v6"
	if [ "$ipv6_enabled" = "1" -a "$ipv6_wan_interface" = "$interface" ];then
		pdptype="ipv4v6"
		[ "`cat /tmp/sim_status 2>/dev/null`" = "ready" -a "`cat /tmp/sim_type 2>/dev/null`" != "verizon" ] && gl_modem -B $modem_bus AT AT+CGDCONT=1,\"IPV4V6\"
	fi
	...
	#fix qmi
	autoconnect=""
	[ "`cat /tmp/sim_type 2>/dev/null`" = "verizon" ] && profile=3 || profile=1
