	...
	case "$service" in
		cdma|evdo)
			if [ "$ipv6_enabled" = "1" -a "$ipv6_wan_interface" = "$interface" ];then
				...
				[ "`cat /tmp/sim_type 2>/dev/null`" = "verizon" ] && chat="/etc/chatscripts/csg_evdoipv6.chat"
			else
		...
		*)
			if [ "$ipv6_enabled" = "1" -a "$ipv6_wan_interface" = "$interface" ];then
				...
				[ "`cat /tmp/sim_type 2>/dev/null`" = "verizon" ] && chat="/etc/chatscripts/csg_3gipv6.chat"
			else
				chat="/etc/chatscripts/3g.chat"
				[ "`cat /tmp/sim_type 2>/dev/null`" = "verizon" ] && chat="/etc/chatscripts/csg_3g.chat"
			fi
			...
			if [ -z "$dialnumber" ]; then
				dialnumber="*99***1#"
				[ "`cat /tmp/sim_type 2>/dev/null`" = "verizon" ] && dialnumber="*99***3#"
			fi
