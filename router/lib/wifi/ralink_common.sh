...
del_br_port() {
    local net=$1
    local port=$2
    local ovs=$(which ovs-vsctl)
    local br

    network_get_device br $net

    [ -z "$br" ] && return

    [ -n "$ovs" ] && $ovs br-exists $br && $ovs del-port $br $port 2>/dev/null
    brctl show $br > /dev/null 2>&1 && brctl delif $br $port 2>/dev/null
}
...
add_br_port() {
    ...
    [ -n "$ovs" ] && $ovs br-exists $br && $ovs add-port $br $port 2>/dev/null || {
        brctl show $br > /dev/null 2>&1 && brctl addif $br $port 2>/dev/null
    }
}
...
enable_ralink_wifi() {
    ...
    for vif in $vifs; do
        ...
        if [ "$disabled" == "1" ]; then
            ...
            del_br_port $network $ifname
            continue
        else
            [ -n "$macaddr" ] && ip link set $ifname addr "$macaddr"
        ...
        [ "$mode" = "sta" ] && {
            ...
            bssid=$(uci -q get wireless.sta.bssid)
            encryption=$(uci -q get wireless.sta.encryption)
            key=$(uci -q get wireless.sta.key)
            channel=$(uci -q get wireless.sta.channel)
            network=$(uci -q get wireless.sta.network)

            json_init
            json_add_string name apcli
            json_add_object instances
            json_add_object instance1
            json_add_array command
            json_add_string "" "/usr/bin/apcli"
            json_add_string "" "-i"
            json_add_string "" "apcli0"
            json_add_string "" "-s"
            json_add_string "" "$ssid"
            json_add_string "" "-n"
            json_add_string "" "$network"
            json_add_string "" "-e"
            json_add_string "" "$encryption"
